...
class Conv2dPad(nn.Conv2d):
    ...
    def vdpConv2d(self, x, weight, bias, stride, padding, dilation, groups):
        if self.input_flag:
            mu_y = self.mu(mu_x)
            vec_x = self.unfold(mu_x)
            sigma_y = self.softplus(self.sigma.weight).repeat(1, vec_x.shape[1]) @ vec_x ** 2
            sigma_y = sigma_y.view(mu_y.shape[0], mu_y.shape[1], mu_y.shape[2], mu_y.shape[3])
            pass
        else:
            mu_y = self.mu(mu_x)
            vec_x = self.unfold(mu_x)
            sigma_y = (self.softplus(self.sigma.weight).repeat(1, vec_x.shape[1]) @ self.unfold(sigma_x)) + \
                      (self.mu.weight.view(self.out_channels, -1)**2 @ self.unfold(sigma_x).T).T + \
                      (self.softplus(self.sigma.weight).repeat(1, vec_x.shape[1]) @ (vec_x ** 2).T).T
            sigma_y = sigma_y.view(mu_y.shape[0], mu_y.shape[1], mu_y.shape[2], mu_y.shape[3])
        return mu_y, sigma_y
