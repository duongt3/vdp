...
class Conv2dDynamicSamePadding(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1, groups=1, bias=True, input_flag=False, image_height=224, image_width=224):
        ...
        ih, iw = image_height, image_width
...
class Conv2dStaticSamePadding(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, image_size=None, input_flag=False, dilation=1, **kwargs):
        ...
        padding = 0
        if pad_h > 0 or pad_w > 0:
            self.padding = (pad_w // 2, pad_w - pad_w // 2,
                            pad_h // 2, pad_h - pad_h // 2)

        self.mu = torch.nn.Conv2d(self.in_channels, self.out_channels,
                                  self.kernel_size, self.stride, self.padding, self.dilation,
                                  self.groups, self.bias, padding_mode="zeros")
        torch.nn.init.xavier_normal_(self.mu.weight)
        self.sigma = torch.nn.Linear(1, out_channels, bias=self.bias)
        torch.nn.init.uniform_(self.sigma.weight, a=0, b=5)

        self.unfold = torch.nn.Unfold(kernel_size, self.dilation, padding, stride)
