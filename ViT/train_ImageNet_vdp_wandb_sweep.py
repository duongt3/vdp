...
class ViT_vdp(SimpleViT_vdp, pl.LightningModule):
    def __init__(self, config, num_classes=1000):
        super().__init__(image_size = 256, patch_size = 32, channels = 3, num_classes = num_classes, dim = 768, depth = 12, heads = 12, mlp_dim = 3072)
        self.lr = config.lr
        self.wd = config.wd
    ...
    def configure_optimizers(self):
        ...
        if self.sched == "multisteplr":
            scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [25, 50, 75])
...
def train(model, epochs):
    ...
    early_stop_callback = EarlyStopping(monitor="loss", min_delta=config.alpha * 0.01, patience=1, verbose=False, mode="min")
    checkpoint_callback = ModelCheckpoint(every_n_epochs=25)
    trainer = pl.Trainer(devices=[0], max_epochs=epochs, check_val_every_n_epoch=5, logger=wandb_logger, #auto_scale_batch_size='power',
                         accelerator='gpu', callbacks=[checkpoint_callback], inference_mode=False)
    trainer.tune(model)
    trainer.fit(model)
    out = trainer.test(model)
    test_acc = out[0]['test_acc_epoch']
    # torch.save(model.state_dict(), 'vit_b_imagenet.pt')
